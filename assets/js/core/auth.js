...
function isValidGmail(email) {
  ...
  /**
 * Enhanced UI Update
 * Automatically populates profile fields if they exist on the current page
 */
function updateAuthUI(user) {
  if (!user) return;

  // 1. Update Header Navigation (Top right)
  const displayNames = document.querySelectorAll(".displayName, .user-name");
  ...
  // 2. Automate Profile Page Info (The "Automation" part)
  // This checks if the user is on the profile page by looking for these specific classes
  const profileNameFields = document.querySelectorAll(".profileName, .fullName");
  const profileEmailFields = document.querySelectorAll(".profileEmail");
  const avatarImage = document.querySelector(".profile-avatar[data-avatar] img");

  // Fill Name
  profileNameFields.forEach(field => {
    field.textContent = user.name || "Guest User";
  });

  // Fill Email
  profileEmailFields.forEach(field => {
    field.textContent = user.email;
  });

  // Fill Avatar if Google provided a picture
  if (avatarImage && user.picture) {
    avatarImage.src = user.picture;
  }

  // 3. Show welcome message on homepage/dashboards
...
document.addEventListener("DOMContentLoaded", function() {
    const user = window.StorageManager.getCurrentUser();
    if (user) {
        // Pre-fill the "Edit" Modal fields so the user doesn't have to re-type
        const nameInput = document.getElementById('bi-name');
        const emailInput = document.getElementById('bi-email');
